# i3-config 🚀

究極に使いやすく、見た目もシンプルでモダンな **i3 Window Manager (i3wm)** の設定ファイルです。  
Debian/Ubuntu, Arch Linux, Fedora 向けの自動セットアップスクリプトが付属しています。

---

## 🌟 特徴

- **直感的なキーバインド**: Super (Mod4 / Windows) キーをプレフィックスとし、Vimスタイル (`h`, `j`, `k`, `l`) でフォーカス・ウィンドウ移動が可能
- **洗練された見た目**: タイトルバーを非表示にし、Gaps (ウィンドウ間の余白) や透明化・影効果 (`picom`) を導入
- **使いやすいランチャー**: `rofi` による高速かつ検索しやすいアプリランチャー
- **マルチメディア対応**: 音量調整・輝度調整・スクリーンショットなどの各種ショートカット対応
- **安全なインストール**: 既存の設定ファイルを自動でバックアップ (`config.backup.YYYYMMDD_HHMMSS`)
- **チートシート付属**: 設定キーバインドをブラウザで確認できる `i3-cheatsheet.html` を同梱

---

## 📦 依存パッケージ

セットアップスクリプトを実行すると、お使いのディストリビューションに応じて以下のパッケージが自動インストールされます。

- **i3** / **i3-wm**: ウィンドウマネージャー本体
- **rofi**: アプリケーションランチャー
- **picom**: コンポジタ (透明化・フェード効果など)
- **feh**: 壁紙設定ツール
- **brightnessctl**: 画面輝度調整
- **pavucontrol** / **pulseaudio-utils**: オーディオ管理
- **curl**: スクリプト実行用

---

## 🛠️ インストール方法

### 方法 1: ワンライナーで自動インストール（推奨）

ターミナルで以下のコマンドを実行するだけで、依存関係のインストール、既存設定のバックアップ、および新しい設定の適用がすべて自動で行われます。

```bash
curl -fsSL https://raw.githubusercontent.com/qlonix/i3-config/main/install.sh | bash
```

### 方法 2: 手動でリポジトリをクローンしてインストール

```bash
# リポジトリのクローン
git clone https://github.com/qlonix/i3-config.git
cd i3-config

# インストールスクリプトの実行
chmod +x install.sh
./install.sh
```

---

## 🔄 設定ファイルのアップデート方法

GitHub上でリポジトリが更新された際、以下のいずれかの方法で手元の環境を簡単にアップデートできます。

### 方法 1: ショートカットキーで更新（推奨）
i3起動中に `Super + Shift + u` を押すだけで、GitHubから最新設定を取得して自動で再読み込み（リロード）されます。

### 方法 2: コマンドラインから更新
ターミナルから以下のいずれかのコマンドを実行します：

```bash
# i3-config-update コマンド（~/.local/bin にパスが通っている場合）
i3-config-update

# またはスクリプトを直接実行
~/.config/i3/update.sh
```

### 方法 3: ワンライナーで更新
リポジトリを手元に保持していない場合でも、ワンライナーで最新化が可能です。

```bash
curl -fsSL https://raw.githubusercontent.com/qlonix/i3-config/main/update.sh | bash
```

---

## ⌨️ キーボード設定・レイアウト変更 (Cinnamon風)

Cinnamonのキーボード設定画面のように、CtrlとCaps Lockの入れ替えやキー配列の変更を対話式GUI/CLIで簡単に行うことができます。

- **設定メニューの起動**:
  - ショートカット: `Super + Ctrl + k`
  - コマンド: `i3-keyboard-setup` (または `~/.config/i3/i3-keyboard-setup.sh`)
- **主な設定項目**:
  - 🔄 **Ctrl と Caps Lock の入れ替え** (`ctrl:swapcaps`)
  - 🔤 **Caps Lock を Ctrl キーに変更** (`ctrl:nocaps`)
  - 🇯🇵 **日本語配列 (jp)**
  - 🇺🇸 **英語配列 (us)**
  - ↺ **デフォルトにリセット**
- **自動保存と永続化**: 選択した設定は `~/.config/i3/keyboard.conf` に保存され、i3の起動時・リロード時に自動適用されます。

---

## 🎨 システムテーマ設定 (ダークモード / ライトモード)

GTK3/GTK4アプリケーションおよび i3wm のステータスバー・ウィンドウボーダーのテーマ（ダーク / ライト）を一括で簡単に切り替えられます。

- **テーマ設定メニューの起動**:
  - ショートカット: `Super + Ctrl + t`
  - コマンド: `i3-theme-setup` (または `~/.config/i3/i3-theme-setup.sh`)
- **主な切替機能**:
  - 🌙 **ダークモード** (`prefer-dark`, `Adwaita-dark`, ダーク i3bar)
  - ☀️ **ライトモード** (`prefer-light`, `Adwaita`, ライト i3bar)
  - 🔄 **ワンタッチ切替**: `i3-theme-setup toggle`
- **一括連動対象**: GTK3/GTK4アプリ (`settings.ini` & `gsettings`), i3bar/i3statusカラーテーマ

---

## 🖼️ デスクトップ壁紙 & ロック画面設定 (ncurses)

ターミナル上の ncurses (dialog) インターフェースから、Linux Mint 公式壁紙や好みの画像、単色カラーをデスクトップ壁紙およびロック画面に一括設定できます。

- **メニューの起動**:
  - ショートカット: `Super + Ctrl + w`
  - コマンド: `i3-wallpaper-setup` (または `~/.config/i3/i3-wallpaper-setup.sh`)
- **主な機能**:
  - 🖼️ **Linux Mint 公式壁紙選択**: インストール済みのMint公式壁紙一覧から即座に選択・適用
  - 📁 **ファイルブラウザ**: `~/Pictures` 等から任意の画像ファイルを選択
  - 🎨 **単色背景**: Catppuccin、Nord、純黒(OLED Black)などのプリセットカラー
  - 🔒 **ロック画面効果**: 視認性を高める「ぼかし加工 (Blur)」、元画像そのまま、単色表示の切替
  - 👁️ **ロック画面案内**: スリープ復帰時にも復帰中であることが一目で分かるロック解除案内テキストを自動合成
- **自動保存と永続化**: `~/.config/i3/wallpaper.conf` に保存され、i3起動時・リロード時に自動適用されます。

---

## 📖 キー操作チートシートの確認方法

ターミナル（コマンドライン）上、またはブラウザ/ショートカットキーから最新のキーバインド一覧をすぐに確認できます。

- **ターミナル (CLI) で確認（新機能）**:
  ```bash
  i3-cheatsheet
  # または
  ~/.config/i3/i3-cheatsheet.sh
  ```
- **ショートカットキーで確認**: `Super + Shift + ?` (または `Super + Shift + /`)
- **ブラウザ (HTML) で確認**: `~/.config/i3/i3-cheatsheet.html` をブラウザで開く

---

## 🚀 ログイン時自動起動アプリケーション設定 (ncurses / dialog)

i3の起動時（ログイン時）に自動起動するシステムデーモンやアプリケーションを、ncurses（`dialog`）の視覚的なチェックボックスで選択・管理できます。

- **設定メニューの起動**:
  - ショートカット: `Super + Ctrl + o`
  - コマンド: `i3-autostart` (または `~/.config/i3/i3-autostart.sh`)
  - 電源メニュー (`Super + Shift + e`) の「🚀 自動起動アプリ設定」からも起動可能
- **主な機能**:
  - **チェックリスト選択**: `picom` (透明化), `nm-applet` (Wi-Fi), `xfce4-power-manager` (電源管理), `fcitx5` (日本語入力), `ulauncher`, 各種ブラウザやチャットツール等の常駐をスペースキーで簡単に ON/OFF。
  - **カスタムコマンド追加**: ユーザー独自の起動コマンドやスクリプトを自由に追加・削除。
  - **二重起動防止**: 設定再読み込み時やセッション復帰時でも、プロセスが多重起動しない安全設計。
  - **設定の保存と即時テスト**: `~/.config/i3/autostart.conf` に永続化され、その場ですぐにテスト起動も可能。

---

## ☀️ 画面輝度調整と最適化 (VAIO Pro / Intel バックライト対応)

ノートPCのFnキーやファンクションキー（`XF86MonBrightnessUp` / `Down`）を押すことで、OSD通知バーとともに滑らかに画面の明るさを調整できます。

- **インテリジェント・バックライト判定 & ハイブリッド・ブースト**:
  - VAIO Pro などの Intel CPU 搭載機では、BIOS経由の `acpi_video0` と GPUネイティブの `intel_backlight` が同時に存在し、`acpi_video0` を操作すると最大輝度が 60〜70% 程度に制限されてしまう現象が発生します。
  - 本設定の `i3-media-control.sh` は、`intel_backlight`（または AMD/NVIDIA/Poulsbo ネイティブドライバ）を自動検出して優先制御するため、**液晶パネル本来の最大輝度 (100%) まで正確に出力**されます。
  - **ソフトウェア輝度ブースト (105%〜130%)**: ハードウェアが物理最大 (100%) に達した状態でさらに `Brightness Up` キーを押すと、自動的にソフトウェアブーストモードへ移行し、最大 130% まで画面をさらに明るく増幅します。
  - **フルカラーレンジ (Broadcast RGB: Full)**: Intel GPU 出力の Limited RGB (16-235) による白くすみ・減光を防止し、0-255 のフルレンジで駆動します。
- **診断・手動操作コマンド**:
  ```bash
  # 画面輝度の状態・認識デバイスを診断
  ~/.config/i3/i3-media-control.sh bright-status

  # 画面を本来の最大輝度 (100%) に瞬時に設定
  ~/.config/i3/i3-media-control.sh bright-max

  # 物理限界を超えて明るくする (120% ブースト)
  ~/.config/i3/i3-media-control.sh bright-boost
  ```
- **💡 VAIO Pro / Intel 機でさらに明るく・くすみを消したい場合 (GRUB設定)**:
  Intel GPUの動的省電力機能 (DPST / PSR) がバックライトを自動で暗くしている場合があります。
  `/etc/default/grub` の `GRUB_CMDLINE_LINUX_DEFAULT` に以下を追記して `sudo update-grub` を実行すると、動的減光を無効化できます：
  ```bash
  GRUB_CMDLINE_LINUX_DEFAULT="... i915.enable_dpst=0 i915.enable_psr=0"
  ```

---

## 📊 ステータスバー（画面下部）の表示項目ガイド

画面下部のバーには、システム状態を示すリアルタイム情報がアイコン付きで表示されます：

| アイコン | 項目名 | 説明・例 |
| :--- | :--- | :--- |
| `🔊` / `🔇` | **音量** | スピーカーの現在音量（ミュート時は `消音` 表示） |
| `💾` | **ストレージ** | メインディスク ( `/` ) の残り空き容量 (例: `124.0 GiB`) |
| `📡` | **Wi-Fi** | 無線LANのIPアドレス・接続SSID・電波強度 (%) |
| `🌐` | **有線LAN** | 有線ネットワーク接続のIPアドレス |
| `⚡` | **CPU使用率** | 現在のプロセッサ使用負荷 (%) |
| `🧠` | **メモリ** | RAMの現在使用量 / 搭載総量 (例: `1.7 GiB / 3.6 GiB`) |
| `🔋` | **バッテリー** | ノートPCの電池残量 (%) と推定残り時間 |
| `📅` `🕒` | **日時** | 現在の年月日・曜日および時刻 |

---

## 📂 ファイル構成

| ファイル名 | 説明 |
| :--- | :--- |
| `config` | i3wm のメイン設定ファイル (`~/.config/i3/config` へ配置) |
| `terminal.conf` | 使用する標準ターミナルの定義ファイル |
| `i3status.conf` | アイコン・日本語表記・カラーテーマを適用したモダンスタイルステータスバー設定 |
| `install.sh` | 依存関係のインストールおよび設定配置を行う自動スクリプト |
| `update.sh` | GitHub上の最新設定を取得・適用しi3をリロードするアップデートスクリプト |
| `i3-default-apps.sh` | デフォルトアプリケーション(ブラウザ/エディタ/ターミナル等)設定ツール |
| `i3-theme-setup.sh` | システム全体のライト/ダークテーマ一括切り替えツール |
| `i3-keyboard-setup.sh` | 対話式キーボードレイアウト & Ctrl/Caps設定ツール |
| `i3-display-setup.sh` | 画面解像度最適化 & Compactモード設定ツール |
| `i3-power-setup.sh` | 電源管理・スリープ待機時間・自動画面ロック設定ツール |
| `i3-wallpaper-setup.sh` | デスクトップ壁紙＆ロック画面設定ツール (ncurses) |
| `i3-autostart.sh` | 起動時自動起動アプリケーション設定ツール (ncurses) |
| `autostart.conf` | 自動起動するアプリケーション・デーモンの設定ファイル |
| `i3-lock.sh` | 壁紙・ぼかし・案内表示付きロック画面ラッパースクリプト |
| `i3-sysmenu.sh` | システム電源メニュー (ロック/スリープ/再起動/終了) |
| `i3-media-control.sh` | メディアキー(音量・輝度)操作時にOSDポップアップ通知を表示するスクリプト |
| `i3-cheatsheet.sh` | ターミナル表示用カラーキーバインドチートシート |
| `i3-cheatsheet.html` | キーバインドを一覧確認できるHTMLチートシート |
| `.agent/RULES.md` | 本プロジェクトの開発・コーディングルール |

---

## ⌨️ 主なショートカット一覧

| ショートカット | 機能 |
| :--- | :--- |
| `Super + Enter` | ターミナル起動 |
| `Super + d` | アプリケーションランチャー (`rofi`) 起動 |
| `Super + Shift + q` | フォーカス中のウィンドウを閉じる |
| `Super + h/j/k/l` | ウィンドウのフォーカス移動 |
| `Super + Shift + h/j/k/l` | ウィンドウの移動 |
| `Super + Shift + c` | i3設定ファイルの再読み込み |
| `Super + Shift + r` | i3の再起動 |
| `Super + Shift + u` | GitHubから最新設定を取得してアップデート |
| `Super + Ctrl + a` | デフォルトアプリケーション (ブラウザ/エディタ/ターミナル等) 設定メニュー |
| `Super + Ctrl + k` | キーボードレイアウト & Ctrl/Caps設定メニュー |
| `Super + Ctrl + t` | システムテーマ (ダーク/ライト) 設定メニュー |
| `Super + Ctrl + s` | 画面最適化 (解像度/Compact切替) 設定メニュー |
| `Super + Ctrl + p` | 電源・省電力・自動画面ロック設定メニュー |
| `Super + Ctrl + w` | 壁紙＆ロック画面設定メニュー (ncurses) |
| `Super + Ctrl + o` | 起動時自動起動アプリケーション設定メニュー (ncurses) |
| `Super + Ctrl + l` | 画面の即時ロック (Lock) |
| `Super + Shift + ?` | ターミナルチートシートの表示 |
| `Super + Shift + e` | システム電源メニュー (ロック/スリープ/再起動/終了) |

---

## 📝 備考

- 設定を反映するために、インストール完了後はログアウトして再ログイン（または `Super + Shift + r` でi3を再起動）してください。
