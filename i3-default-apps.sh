...
configure_category() {
    ...
    if [ -n "$current_desktop" ]; then
        for dir in "/usr/share/applications" "$HOME/.local/share/applications" "/var/lib/snapd/desktop/applications" "/var/lib/flatpak/exports/share/applications"; do
            if [ -f "$dir/$current_desktop" ]; then
                current_name=$(get_app_name "$dir/$current_desktop")
                break
            fi
        done
        if [ "$current_name" = "未設定" ]; then
            current_name="$current_desktop"
        fi
    fi
            ...
            for f in "$dir"/*.desktop; do
                ...
                if grep -q "$primary_mime" "$f" 2>/dev/null || grep -q "Categories=.*$category_tag" "$f" 2>/dev/null; then
                    ...
                    if [[ ! " ${options[*]} " =~ " ${base} " ]]; then
                        desktop_list+=("$base|$app_title")
                        options+=("$app_title ($base)")
                    fi
                fi
            done
        fi
    ...
    if [ -n "$selected" ]; then
        ...
        local chosen_title=$(echo "$selected" | sed 's/ (.*)//')
...
configure_terminal() {
    ...
    local selected=""
    if command -v rofi &>/dev/null && [ -n "$DISPLAY" ]; then
        selected=$(printf '%s\n' "${installed[@]}" | rofi -dmenu -i -p "💻 標準ターミナル選択 (現在: $current_term)")
    else
        ...
        read -p "選択番号を入力してください: " num
        if [ "$num" -ge 1 ] && [ "$num" -le "${#installed[@]}" ]; then
    ...
    if [ -n "$selected" ]; then
        ...
        echo "set \$term $selected" >> "$TERMINAL_CONF"
        if command -v i3-msg &>/dev/null && pgrep -x i3 &>/dev/null; then
            i3-msg reload >/dev/null 2>&1 || true
        fi
        if command -v notify-send &>/dev/null && [ -n "$DISPLAY" ]; then
            notify-send -h "string:x-dunst-stack-tag:default-app" -t 2000 "✅ ターミナル変更" "標準ターミナルを「$selected」に設定しました"
        else
            echo "✅ 標準ターミナルを「$selected」に設定しました"
        fi
    fi
}
...
main_menu() {
    local menu_items=(
        ...
        "🖼️ 画像ビューア (Image Viewer)"
        "🎬 動画・音楽プレーヤー (Media Player)"
        "✉️ メールクライアント (Mail Client)"
        "💻 標準ターミナル (Terminal Emulator)"
    )
